...
price_model = joblib.load(BASE_DIR / 'rf_price_prediction.pkl')
city_enc = joblib.load(BASE_DIR / 'city_encoder.pkl')
state_enc = joblib.load(BASE_DIR / 'state_encoder.pkl')
investment_model = joblib.load(BASE_DIR / 'investment_label_model.pkl')
roi_model = joblib.load(BASE_DIR / 'ROI_predictor.pkl')
...
def predict_invest(features : pricehousefeatures):
    try: 
        ...
        return {'price': predicted_price,'Investment_label' : label, 'ROI_score' : roi_score, 'Note':note}
    except Exception as e:
        raise HTTPException (status_code = 500, detail = str(e))
